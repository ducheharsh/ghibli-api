...
import random
import sys
from flask import Flask, jsonify, request, redirect, abort, render_template, send_from_directory
import logging
...
def index():
    ...
    random_image = None
    if database["images"]:
        random_image = random.choice(database["images"])
    
    # Get a few sample images for the gallery section
    sample_images = []
    if database["images"]:
        sample_images = random.sample(database["images"], min(9, len(database["images"])))
    
    # Get film codes for the dropdown
